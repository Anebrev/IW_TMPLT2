...
procedure TIWFRM_Consolida.ListarAliquotas_v1;
var
  i: Integer;
  colspan, aliq: string;
...
begin
  ...
  begin
    ...
    qtAlteracao:= UserSession.DM.qtAltAliq(StrToFloat(aliq)).ToString;
    tagCol1 := tagCol1 + '<th colspan="'+colspan+'" style="text-align: center;">'+
    //aliq.Replace(',','.')+'<span class="numberCircle clrRose"><span>'+qtAlteracao+'</span></span> </th> '+SLineBreak;
    aliq.Replace(',','.')+addTagAliq(StrToInt(qtAlteracao))+' </th> '+SLineBreak;
    lQue.Next;
  end;
  lQue.Close;

  tagCol1 := tagCol1 + '<th rowspan="2" colspan="1" style="text-align: center;">Ações</th> '+SLineBreak;
  ...
  begin
   ...
   lQue3 := UserSession.DM.ListarAliquotas();
   tabtd:='';
   tabela:='';
   lQue3.First;
   ...
   begin
     ...
     for i := 2 to lQue3.Fields.Count -2 do
     begin
       case i  of
  ...
  html := '';
  html :=  '<table id="DTB_TAG" class="table table-bordered table-striped table-hover"> '+SLineBreak+
           '    <thead> '+SLineBreak+
           '        <tr> '+SLineBreak+
...
begin
  ...
  begin
    ...
    aliq    := lQue.FieldByName('PPSICM').AsString;
    qtAlteracao:= UserSession.DM.qtAltAliq(StrToFloat(aliq)).ToString;
    tagCol1 := tagCol1 + '<th colspan="'+colspan+'" id="fitPadLL">'+
    aliq.Replace(',','.')+addTagAliq(StrToInt(qtAlteracao))+' </th> '+SLineBreak;
    lQue.Next;
  end;
  lQue.Close;
  ...
  begin
   ...
   while NOT lQue3.Eof do
   begin
     { montando chave para o botao historico... }
     IDP_CDE_CDP := trim(lQue3.FieldByName('PPSID').AsString)+
                    lQue3.FieldByName('PPSEMP').AsString+
                    trim(lQue3.FieldByName('PPSPRD').AsString)+
                    lQue3.FieldByName('PPSVIGINI').AsString +
     ...
     { adicionando os campos de valores }
     //for i := 2 to lQue3.Fields.Count -1 do
     for i := 2 to lQue3.Fields.Count -3 do  //***********altered from -1 to -3
     begin
       case i  of
           2 : campo:= lQue3.Fields[i].AsString.Trim;
         3,4 : campo:= Concat(Copy(lQue3.Fields[i].AsString,7,2),'/',Copy(lQue3.Fields[i].AsString,5,2),'/',Copy(lQue3.Fields[i].AsString,1,4)); //YYYYMMDD -> DD/MM/YYYY
       ...
       linTemAlt:= lQue3.RecNo; //***criar campo qtAlteracao (linha)
       colorClass:= '';
       If linTemAlt = 3 then
         colorClass:= 'hasChange';
       tabtd:= tabtd + ' <td id="fitPad" class="'+colorClass+'"> '+campo+' </td> '+ SLineBreak;
       //tabtd:= tabtd + ' <td id="fitPad" class='+'"valor'+campo.Replace('.','')+'"> '+campo+' </td> '+ SLineBreak;
     end;
